...
async function routeCall(audioStream) {
  ...
  return operator.transfer(intent.department);
    ...
    return (
        <section id="projects" className="py-32 px-6 bg-surface-light">
            <div className="container mx-auto px-6">
                <div className="mb-24 text-center">
                    <p className="text-[11px] font-bold text-accent uppercase tracking-[0.3em] mb-4">Case Studies</p>
                ...
                <div className="space-y-40">
                    {projects.map((project, index) => (
                        <div key={index} className="grid lg:grid-cols-12 gap-16 items-center project-card p-4 rounded-5xl hover:bg-white/50 transition-colors duration-500">
                            <div className="lg:col-span-6 order-2 lg:order-1">
                                <div className="inline-flex gap-3 mb-8">
                                    <span className="text-[11px] font-bold text-accent uppercase tracking-widest px-4 py-2 bg-accent/10 rounded-full">{project.tag}</span>
                                    <span className="text-[11px] font-bold text-slate-500 uppercase tracking-widest px-4 py-2 bg-slate-200/50 rounded-full">{project.type}</span>
                                </div>
                                <h3 className="text-4xl md:text-5xl font-bold text-primary mb-8">{project.title}</h3>
                                <p className="text-charcoal mb-10 text-xl leading-relaxed">
                                    {project.description}
                                </p>
                                <div className="flex items-center gap-10 border-t border-border-subtle pt-10">
                            ...
                            <div className="lg:col-span-6 order-1 lg:order-2">
                                <div className="bg-primary rounded-[3rem] p-10 shadow-2xl overflow-hidden relative border border-slate-800">
                                    <div className="mb-6 flex items-center justify-between text-[11px] font-mono text-slate-500">
                                        <span>Architecture v2.4</span>
                                        <span className="text-accent flex items-center gap-2">
                                            <span className="w-2 h-2 rounded-full bg-accent animate-pulse"></span>ACTIVE
                                        </span>
                                    </div>
                                    <div className="bg-[#020617] border border-white/5 p-8 rounded-2xl font-mono text-xs leading-loose text-slate-300">
                                        <pre className="whitespace-pre-wrap">{project.code}</pre>
                                    </div>
                                    <div className="absolute -right-12 -bottom-12 opacity-10">
                                        <span className="material-symbols-outlined text-[200px] text-white">{project.icon}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
