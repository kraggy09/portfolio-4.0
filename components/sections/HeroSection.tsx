...
export default function HeroSection({ t }: { t: TranslationDict }) {
    const router = useRouter();
    const [isGlitching, setIsGlitching] = useState(false);

    const triggerEasterEgg = () => {
        setIsGlitching(true);
        // Play a glitch sound if possible, but for now just visual
        setTimeout(() => {
            router.push("/system-override");
        }, 800);
    };

    return (
        <section className={`relative min-h-screen flex flex-col justify-center py-20 md:py-0 px-4 md:px-6 overflow-hidden bg-white ${isGlitching ? 'glitch-active' : ''}`}>
            {/* Glitch Overlay */}
            <div className="glitch-overlay"></div>

            {/* Neural Background Detail */}
            <div className="absolute top-0 right-0 w-1/2 h-full opacity-30 select-none pointer-events-none overflow-hidden">
                <div className="neural-ring absolute top-1/4 right-0 w-[80vw] md:w-[600px] h-[80vw] md:h-[600px] border border-blue-500/10 rounded-full animate-rotate-slow translate-x-1/4"></div>
                <div className="neural-ring absolute top-1/3 right-10 w-[60vw] md:w-[500px] h-[60vw] md:h-[500px] border border-blue-500/5 rounded-full animate-rotate-reverse translate-x-1/4"></div>
            </div>

            <div className="container mx-auto grid lg:grid-cols-2 gap-12 lg:gap-24 items-center relative z-10">
                <div className="pt-10 lg:pt-0">
                    ...
                    <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-border-subtle bg-white text-xs font-bold uppercase tracking-widest text-slate-500 mb-6 shadow-sm">
                        <span className="w-2 h-2 rounded-full bg-accent animate-pulse"></span>
                        <span>{t.hero.tag}</span>
                    </div>
                    <p className="text-3xl md:text-6xl font-bold text-primary leading-[1.1] mb-8">
                        {t.hero.title} <br />
                    ...
                    <div className="flex flex-wrap gap-4">
                        <a
                            href="#work"
                            className="px-8 py-4 bg-primary text-white text-xs font-bold uppercase tracking-widest rounded-full hover:bg-accent transition-all shadow-xl hover:shadow-accent/20"
                        >
                            {t.hero.cta}
                        </a>
                        <div className="flex items-center gap-3 px-8 py-4 rounded-full border border-border-subtle text-xs font-bold uppercase tracking-widest text-slate-400">
                            <span className="w-2 h-2 rounded-full bg-accent animate-pulse"></span>
                            {t.hero.availability}
                        </div>
                    </div>
                ...
                <div className="relative flex justify-center items-center h-[320px] md:h-[450px]">
                    <div
                        className="relative w-full max-w-sm md:max-w-md aspect-square transition-transform duration-200 ease-out cursor-pointer group"
                        id="neural-core"
                    ...
                    >
                        <div className="absolute inset-0 flex items-center justify-center scale-90 md:scale-100">
                            ...
                            <div className="relative z-20 w-10 md:w-12 h-10 md:h-12 rounded-full bg-primary border-4 border-accent/30 flex items-center justify-center pulse-nucleus group-hover:scale-110 transition-transform">
