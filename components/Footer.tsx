...
export default function Footer({ t, language }: FooterProps) {
    ...
    return (
        <footer className="py-12 px-6 bg-transparent border-t border-border w-full relative overflow-hidden font-sans">
            {/* Background Flair */}
            <div className="absolute top-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-accent/10 to-transparent"></div>
            <div className="absolute -bottom-24 -left-24 w-64 h-64 bg-accent/[0.02] rounded-full blur-[100px] pointer-events-none"></div>

            <div className="max-w-[680px] mx-auto">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 mb-10 text-[13px]">
                    {/* Identity Section */}
                    <div>
                        <div className="flex items-center gap-3 mb-4 group cursor-default text-text">
                        ...
                        <p className="text-dim text-[10px] font-mono uppercase tracking-[0.1em]">
                    ...
                    {/* Connect Section */}
                    <div>
                        <span className="text-[10px] font-medium text-dim uppercase tracking-[0.2em] block mb-3">
                            Connect
                        </span>
                        <div className="flex flex-col gap-2">
                            <a href="https://linkedin.com/in/mkaifshaikh" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-muted hover:text-accent transition-all group text-[13px]">
                            ...
                            <a href="https://github.com/kraggy09" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-muted hover:text-accent transition-all group text-[13px]">
                                <svg viewBox="0 0 24 24" className="w-3.5 h-3.5 fill-current transition-transform group-hover:translate-x-0.5" xmlns="http://www.w3.org/2000/svg">
                    ...
                    {/* Status Section */}
                    <div className="bg-tag-bg border border-border p-4 rounded-xl shadow-sm">
                        <span className="text-[10px] font-medium text-dim uppercase tracking-[0.2em] block mb-2.5">
                            Status
                        </span>
                        <div className="space-y-2">
                            <div className="flex items-center gap-2.5">
                                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
                                <span className="text-[12px] font-medium text-text tracking-wide">
                ...
                <div className="pt-8 border-t border-border flex flex-col sm:flex-row items-center justify-between gap-4">
                    <p className="text-[10px] font-medium text-dim tracking-widest uppercase">
                        {t.footer?.copyright || `© ${new Date().getFullYear()} KAIF SHAIKH`}
                    </p>
                    <div className="flex items-center gap-6 text-[10px] font-medium text-dim tracking-wider uppercase">
                        <button
                            onClick={scrollToTop}
